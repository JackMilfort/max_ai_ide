...
const state = {
  ...
  _selectedModelos: [],             // [{model, endpoint, endpointId, name}, ...]
  ...
  _cachedModelos: [],               // cached model list for pane dropdowns
  ...
  _searchSynthModelos: null,        // per-pane synthesis models for search mode
  _parallel: true,                 // true = run all panes at once, false = one at a time
  _fetchModelosCache: null,
  _fetchModelosCacheTime: 0,
...
export function reset() {
  ...
  state._lastPrompt = '';
  state._cachedModelos = [];
  state._probed = new Set();
  state._cachedProviders = null;
  state._fetchModelosCache = null;
  state._fetchModelosCacheTime = 0;
}
