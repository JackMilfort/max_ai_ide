 ...
 *   #ge-import-gallery   Galería picker — fetches /api/gallery/library
 *                        and shows a thumbnail grid overlay
 *
 * Plus the shared `handleImportedImage(img)` sink — scales to canvas,
 * centres, creates a new layer, switches to Mover tool, hides the
...
export function wireImport({ container, saveState, createLayer, composite, renderLayerPanel, uiModule }) {
  ...
  // Import from Galería — fetch /api/gallery/library and show a
  ...
  document.getElementById('ge-import-gallery')?.addEventListener('click', async () => {
    try {
      ...
      panel.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;"><span style="font-size:13px;font-weight:600;">Pick from Galería</span><button id="ge-gallery-close" style="background:none;border:none;color:var(--fg);cursor:pointer;font-size:18px;">✕</button></div>';
