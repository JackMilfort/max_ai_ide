...
/**
 * Modelo and provider management
 */
...
function _buildModeloRow(mid, url, displayName, endpointId, offline, modelType) {
  ...
  if (!offline) {
    ...
    row.addEventListener('touchstart', () => { _touchMoverd = false; }, { passive: true });
    row.addEventListener('touchmove', () => { _touchMoverd = true; }, { passive: true });
    row.addEventListener('click', (e) => {
      if (e.target.closest('.item-drag-handle') || e.target.closest('.model-fav-btn')) return;
      if (_touchMoverd) { _touchMoverd = false; return; }
      _startChat(url, mid, endpointId);
    });
  }
...
export async function refreshModelos(force = false) {
  ...
  try {
    ...
    if (_cachedItems && _cachedItems.length > 0) {
      _cachedItems.forEach(item => {
        ...
        if (!extraGrupos[cat][epName]) extraGrupos[cat][epName] = [];
        const displayNames = item.models_display || item.models || [];
        const epModeloType = item.model_type || 'llm';
    ...
    if (favs.length > 0) {
      ...
      for (const cat of ['local', 'api']) {
        for (const [epName, epModelos] of Object.entries(groups[cat])) {
          for (const m of epModelos) {
      ...
      if (favSort === 'alpha') {
        favModelos.sort((a, b) => a.displayName.split('/').pop().localeCompare(b.displayName.split('/').pop()));
      } else if (favSort === 'last-used') {
        const usage = _loadUsage();
        favModelos.sort((a, b) => ((usage[b.mid] || {}).last || 0) - ((usage[a.mid] || {}).last || 0));
      } else if (favSort === 'most-used') {
        const usage = _loadUsage();
        favModelos.sort((a, b) => ((usage[b.mid] || {}).count || 0) - ((usage[a.mid] || {}).count || 0));
      } else {
        favModelos.sort((a, b) => favs.indexOf(a.mid) - favs.indexOf(b.mid));
      }

      if (favModelos.length > 0) {
        ...
        favCount.textContent = '(' + favModelos.length + ')';
        ...
        if (!favCollapsed) {
          ...
          favModelos.forEach(({ mid, url, displayName, endpointId, offline, modelType }) => {
            favContainer.appendChild(_buildModeloRow(mid, url, displayName, endpointId, offline, modelType));
          });
    ...
    categoryOrder.forEach(({ key, label }) => {
      ...
      // --- Endpoint sub-groups ---
      const extraEndpoints = extraGrupos[key];
      Object.entries(endpoints).forEach(([epName, epModelos]) => {
        const epExtra = extraEndpoints[epName] || [];
        const totalCount = epModelos.length + epExtra.length;
        const isOfflineEndpoint = epModelos.length > 0 && epModelos[0].offline;
        ...
        if (sortMode === 'alpha') {
          epModelos.sort((a, b) => a.displayName.split('/').pop().localeCompare(b.displayName.split('/').pop()));
        } else if (sortMode === 'last-used') {
          const usage = _loadUsage();
          epModelos.sort((a, b) => ((usage[b.mid] || {}).last || 0) - ((usage[a.mid] || {}).last || 0));
        } else if (sortMode === 'most-used') {
          const usage = _loadUsage();
          epModelos.sort((a, b) => ((usage[b.mid] || {}).count || 0) - ((usage[a.mid] || {}).count || 0));
        }

        // Show up to MAX_VISIBLE models, rest behind "show more"
        const MAX_VISIBLE = 5;
        const visible = epModelos.slice(0, MAX_VISIBLE);
        const overflow = epModelos.slice(MAX_VISIBLE);
        const allHidden = [...overflow, ...epExtra];

        visible.forEach(({ mid, url, displayName, endpointId, offline, modelType }) => {
          target.appendChild(_buildModeloRow(mid, url, displayName, endpointId, offline, modelType));
        });
    ...
    // Restaurar saved drag order for flat list before enabling drag-sort
    if (!needsGrouping) {
      const savedModeloOrder = Storage.getJSON('models-order', []);
      if (savedModeloOrder.length) {
    ...
    // ── Buscar box (shown when >= 5 total models, including hidden overflow) ──
    const totalModeloCount = (_cachedItems || []).reduce((n, item) => {
      if (item.offline) return n;
      return n + (item.models || []).length + (item.models_extra || []).length;
    }, 0);
    if (totalModeloCount >= 10) {
      const searchBox = document.createElement('input');
      searchBox.type = 'text';
      searchBox.placeholder = 'Buscar models...';
        ...
        (_cachedItems || []).forEach(item => {
          ...
          allModelos.forEach((mid, i) => {
            ...
            searchResults.appendChild(
              _buildModeloRow(mid, item.url, display, item.endpoint_id || null, false, item.model_type || 'llm')
            );
          });
        });
    ...
    if (!_cachedItems || _cachedItems.length === 0) {
      const noModelos = document.createElement('div');
      noModelos.className = 'models-empty-state';
      if (window._isAdmin) {
        noModelos.innerHTML = '<span class="muted">No models found</span><br>'
        ...
        noModelos.innerHTML = '<span class="muted">No models available</span><br>'
          + '<span class="muted-sm">Ask an admin to configure model endpoints</span>';
      }
      box.appendChild(noModelos);
...
export async function refreshProviders() {
  ...
  if (!sel) return; // Salir if element doesn't exist
  ...
  try {
    ...
    if (openai) {
      ...
      sortModeloIds(models).forEach(m => {
